...
void        globject::init(float sizeX, float sizeY) {
    ...
    globject("assets/render/models/particle.obj", PARTICLE, 10);
    ...
    Particle * t = new Particle(0, 0, PARTICLE, PARTICLE);
    //if (t != NULL)
...
void		globject::render_all(Entity ***map, std::list<Entity*> players, SDL_Surface *menu) {
    ...
    while (itP != iteP) {
        ...
        modelPos.z += 0.01;
        modelPos.y = (*itP)->pos_y;
        modelPos.z = (*itP)->pos_z;
        modelPos.x = (*itP)->pos_x;
        Model = Matrix::model_matrix(modelPos, modelDir, \
